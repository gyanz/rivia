...
def _make_synthetic_hdf(path: Path, n_cells: int = 10, n_faces: int = 20,
                         n_timesteps: int = 5, area_name: str = "TestArea") -> None:
    ...
    with h5py.File(path, "w") as f:
        ...
        all_cells = n_cells + 2
        g.create_dataset("Cells Center Coordinate",
                         data=rng.uniform(0, 100, (all_cells, 2)).astype("f8"))
        g.create_dataset("Cells Minimum Elevation",
                         data=rng.uniform(0, 5, (all_cells,)).astype("f4"))
        g.create_dataset("Cells Center Manning's n",
                         data=np.full(all_cells, 0.04, "f4"))
        g.create_dataset("Cells Surface Area",
                         data=rng.uniform(10, 100, (all_cells,)).astype("f4"))
        ...
        g.create_dataset("Cells FacePoint Indexes",
                         data=np.zeros((all_cells, 8), dtype="i4"))
        ...
        g.create_dataset("Faces Minimum Elevation",
                         data=rng.uniform(0, 3, n_faces).astype("f4"))
        g.create_dataset("Faces FacePoint Indexes",
                         data=np.zeros((n_faces, 2), dtype="i4"))
        g.create_dataset("FacePoints Coordinate",
        ...
        ts_root.create_dataset("Time Date Stamp",
                               data=np.array(stamps, dtype="S19"))
        ts_root.create_dataset("Time",
                               data=np.linspace(0, 4, n_timesteps))
        ts_root.create_dataset("Time Step",
                               data=np.ones(n_timesteps, dtype="f4"))
        ...
        area_sum.create_dataset("Maximum Water Surface",
                                data=max_wse.astype("f4"))
        area_sum.create_dataset("Minimum Water Surface",
                                data=min_wse.astype("f4"))
