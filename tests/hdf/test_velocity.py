...
class TestInterpolateFaceFlowArea:
    ...
    def simple_table(self):
        ...
        ae_values = np.array([[0.0, 0.0, 0.0, 0.04],
                              [10.0, 100.0, 40.0, 0.04]], dtype="f4")
...
class TestComputeAllCellVelocities:
    ...
    def simple_mesh(self):
        ...
        normals = np.array([[1.0, 0.0, 10.0],
                            [-1.0, 0.0, 10.0]], dtype="f4")

        fci = np.array([[0, 1], [1, 0]], dtype="i4")

        # flat area-elevation tables: area=50 everywhere
        ae_info   = np.array([[0, 2], [2, 2]], dtype="i4")
        ae_values = np.array([[0.0, 50.0, 20.0, 0.04],
                              [10.0, 50.0, 20.0, 0.04],
                              [0.0, 50.0, 20.0, 0.04],
                              [10.0, 50.0, 20.0, 0.04]], dtype="f4")

        face_vel  = np.array([1.0, -1.0], dtype="f4")
        cell_wse  = np.array([5.0, 5.0], dtype="f4")

        return dict(
            n_cells=n_cells, cf_info=cf_info, cf_values=cf_values,
            normals=normals, fci=fci,
            ae_info=ae_info, ae_values=ae_values,
            face_vel=face_vel, cell_wse=cell_wse,
        )
    ...
    def test_output_shape(self, simple_mesh):
        ...
        result = compute_all_cell_velocities(
            n_cells=m["n_cells"],
            cell_face_info=m["cf_info"], cell_face_values=m["cf_values"],
            face_normals=m["normals"], face_cell_indexes=m["fci"],
            face_ae_info=m["ae_info"], face_ae_values=m["ae_values"],
            face_vel=m["face_vel"], cell_wse=m["cell_wse"],
        )
        assert result.shape == (m["n_cells"], 2)

    def test_area_weighted_and_length_weighted_agree_for_constant_area(self, simple_mesh):
    ...
    def test_sloped_symmetric_agrees_with_average(self, simple_mesh):
        ...
        sloped = compute_all_cell_velocities(
            **kw, wse_interp="sloped",
            cell_coords=cell_coords, face_coords=face_coords,
        )
