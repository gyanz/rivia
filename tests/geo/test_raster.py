...
class TestScalarRaster:
    ...
    def test_nodata_written(self, tmp_path):
        ...
        pts, vals = _scattered_scalar()
        nodata = -999.0
        out = points_to_raster(pts, vals, tmp_path / "out.tif",
                               cell_size=5.0, nodata=nodata)
        with rasterio.open(out) as src:
...
class TestGridAlignment:
    def test_transform_snaps_origin(self, tmp_path):
        ...
        out = points_to_raster(pts, vals, tmp_path / "snapped.tif",
    ...
    def test_reference_raster_inherits_crs(self, tmp_path):
        ...
        pts, vals = _scattered_scalar()
        out = points_to_raster(pts, vals, tmp_path / "aligned.tif",
    ...
    def test_reference_raster_crs_overridden(self, tmp_path):
        ...
        pts, vals = _scattered_scalar()
        out = points_to_raster(pts, vals, tmp_path / "override.tif",
                               reference_raster=ref_path, crs="EPSG:4326")
        with rasterio.open(out) as src:
            assert src.crs.to_epsg() == 4326
...
class TestMinValueMask:
    def test_raises_when_too_few_points_remain(self, tmp_path):
        ...
        with pytest.raises(ValueError, match="Fewer than 3"):
            points_to_raster(pts, vals, tmp_path / "out.tif",
                             cell_size=1.0, min_value=1.0)
    ...
    def test_dry_cells_excluded(self, tmp_path):
        ...
        vals = np.concatenate([rng.uniform(0, 0.5, 25),
                               rng.uniform(2.0, 5.0, 25)])
        # Should not raise; dry cells are simply excluded
        out = points_to_raster(pts, vals, tmp_path / "masked.tif",
                               cell_size=10.0, min_value=1.0)
        assert out.exists()
