"""Tests for rivia.model.flow_steady — SteadyFlow.
...
from rivia.model.flow_steady import SteadyBoundary, SteadyFlow
...
class TestConstruction:
    def test_missing_file_raises(self, tmp_path):
        with pytest.raises(FileNotFoundError):
            SteadyFlow(tmp_path / "missing.f01")

    def test_accepts_str_path(self):
        f = SteadyFlow(str(BAXTER))
...
class TestScalarProperties:
    def test_flow_title_baxter(self):
        assert SteadyFlow(BAXTER).flow_title == "Steady Flows"

    def test_flow_title_mixed(self):
        assert SteadyFlow(MIXED).flow_title == "Flow data with two profiles"

    def test_program_version_modern(self):
        assert SteadyFlow(BAXTER).program_version == "6.30"

    def test_program_version_legacy_version_key(self):
        # WAILUPE uses "Version=Version 3.0 Beta" not "Program Version="
        v = SteadyFlow(WAILUPE).program_version
        assert v is not None
        assert "3.0" in v

    def test_n_profiles_baxter(self):
        assert SteadyFlow(BAXTER).n_profiles == 3

    def test_n_profiles_conspan(self):
        assert SteadyFlow(CONSPAN).n_profiles == 4

    def test_n_profiles_mixed(self):
        assert SteadyFlow(MIXED).n_profiles == 2

    def test_profile_names_baxter(self):
        assert SteadyFlow(BAXTER).profile_names == ["Big", "Bigger", "Biggest"]

    def test_profile_names_conspan(self):
        names = SteadyFlow(CONSPAN).profile_names
        assert len(names) == 4
        assert names[0].strip() == "5 yr"

    def test_profile_names_mixed(self):
        assert SteadyFlow(MIXED).profile_names == ["PF 1", "PF 2"]
    ...
    def test_set_flow_title(self, tmp_copy):
        ...
        f.save()
        assert SteadyFlow(dst).flow_title == "Modified Title"
    ...
    def test_set_profile_names(self, tmp_copy):
        ...
        f.save()
        assert SteadyFlow(dst).profile_names == ["Q2", "Q10", "Q100"]
    ...
    def test_set_n_profiles(self, tmp_copy):
        ...
        f.n_profiles = 5
        f.save()
        assert SteadyFlow(dst).n_profiles == 5
...
class TestGetFlows:
    def test_baxter_upper_reach_count(self):
        vals = SteadyFlow(BAXTER).get_flows("Baxter River", "Upper Reach", "84816.")
        assert len(vals) == 3
    ...
    def test_tule_creek_first_value(self):
        vals = SteadyFlow(BAXTER).get_flows("Tule Creek", "Tributary", "10982.")
        assert vals[0] == pytest.approx(499.9999)

    def test_conspan_four_profiles(self):
        vals = SteadyFlow(CONSPAN).get_flows(
    ...
    def test_missing_location_returns_none(self):
        assert SteadyFlow(BAXTER).get_flows("No River", "No Reach", "0") is None

    def test_wailupe_multiple_locations_for_same_reach(self):
        f = SteadyFlow(WAILUPE)
...
class TestSetFlows:
    def test_set_same_count(self, tmp_copy):
        ...
        f.save()
        result = SteadyFlow(dst).get_flows("Baxter River", "Upper Reach", "84816.")
    ...
    def test_set_does_not_affect_other_location(self, tmp_copy):
        ...
        f.save()
        after = SteadyFlow(dst).get_flows("Tule Creek", "Tributary", "10982.")
    ...
    def test_set_conspan_four_profiles(self, tmp_copy):
        ...
        f.save()
        result = SteadyFlow(dst).get_flows("Spring Creek", "Culvrt Reach", "20.535")
        assert result == pytest.approx(new_vals, rel=1e-5)
...
class TestGetBoundary:
    def test_baxter_upper_up_type_none(self):
        bc = SteadyFlow(BAXTER).get_boundary("Baxter River", "Upper Reach", 1)
    ...
    def test_baxter_lower_dn_type_normal_depth(self):
        bc = SteadyFlow(BAXTER).get_boundary("Baxter River", "Lower Reach", 1)
        assert bc is not None
        assert bc.dn_type == 3
    ...
    def test_conspan_known_ws_profile_1(self):
        bc = SteadyFlow(CONSPAN).get_boundary("Spring Creek", "Culvrt Reach", 1)
        assert bc is not None
        assert bc.dn_type == 1
        assert bc.dn_known_ws == pytest.approx(28.9)

    def test_conspan_known_ws_profile_4(self):
        bc = SteadyFlow(CONSPAN).get_boundary("Spring Creek", "Culvrt Reach", 4)
        assert bc.dn_known_ws == pytest.approx(31.5)

    def test_mixed_up_type_normal_depth(self):
        bc = SteadyFlow(MIXED).get_boundary("Mixed Reach", "Mixed Reach", 1)
        assert bc.up_type == 3
    ...
    def test_missing_boundary_returns_none(self):
        assert SteadyFlow(BAXTER).get_boundary("No River", "No Reach", 1) is None

    def test_missing_profile_number_returns_none(self):
        assert (
            SteadyFlow(BAXTER).get_boundary("Baxter River", "Lower Reach", 99)
            is None
        )

    def test_profile_field_set_correctly(self):
        bc = SteadyFlow(BAXTER).get_boundary("Baxter River", "Lower Reach", 2)
        assert bc.profile == 2
...
class TestGetBoundaries:
    def test_baxter_lower_returns_three_profiles(self):
        bcs = SteadyFlow(BAXTER).get_boundaries("Baxter River", "Lower Reach")
        assert len(bcs) == 3
    ...
    def test_conspan_four_profiles(self):
        bcs = SteadyFlow(CONSPAN).get_boundaries("Spring Creek", "Culvrt Reach")
        assert len(bcs) == 4

    def test_missing_reach_returns_empty(self):
        assert SteadyFlow(BAXTER).get_boundaries("No River", "No Reach") == []
...
class TestSetBoundary:
    def test_set_dn_slope(self, tmp_copy):
        ...
        f.set_boundary(bc)
        f.save()
        result = SteadyFlow(dst).get_boundary("Baxter River", "Lower Reach", 1)
    ...
    def test_set_changes_only_target_profile(self, tmp_copy):
        ...
        bc2 = SteadyFlow(dst).get_boundary("Baxter River", "Lower Reach", 2)
        bc3 = SteadyFlow(dst).get_boundary("Baxter River", "Lower Reach", 3)
    ...
    def test_change_bc_type_from_known_ws_to_normal_depth(self, tmp_copy):
        ...
        f.set_boundary(bc)
        f.save()
        result = SteadyFlow(dst).get_boundary("Spring Creek", "Culvrt Reach", 1)
    ...
    def test_set_does_not_affect_flows(self, tmp_copy):
        ...
        f.set_boundary(bc)
        f.save()
        after = SteadyFlow(dst).get_flows("Baxter River", "Upper Reach", "84816.")
        assert before == pytest.approx(after, rel=1e-5)
    ...
    def test_set_preserves_other_boundaries_in_file(self, tmp_copy):
        ...
        tc = SteadyFlow(dst).get_boundary("Tule Creek", "Tributary", 1)
        assert tc.dn_type == 0
    ...
    def test_set_known_ws(self, tmp_copy):
        ...
        f.set_boundary(bc)
        f.save()
        result = SteadyFlow(dst).get_boundary("Spring Creek", "Culvrt Reach", 2)
...
class TestEscapeHatch:
    def test_get_existing_key(self):
        assert SteadyFlow(BAXTER).get("Flow Title") == "Steady Flows"

    def test_get_missing_key_returns_none(self):
        assert SteadyFlow(BAXTER).get("Nonexistent Key") is None
    ...
    def test_set_existing_key(self, tmp_copy):
        ...
        f.save()
        assert SteadyFlow(dst).get("Flow Title") == "New Title"
