    ...
    if render_mode == "horizontal":
        ...
        if render_mode == "sloping_corners_faces_shallow":
            ...
            if tree is not None:
                ...
                pixel_is_shallow = np.zeros(xi_grid.size, dtype=bool)
                pixel_is_shallow[in_mesh] = cell_is_shallow[
                    wet_indices[idx_flat[in_mesh]]
                ]
...
"sloping_corners_faces_shallow"}
    ...
    method : str, default ``"scatter_interp2"``
        ...
        ``"triangle_blend"``
            ...
            with the face's full 2D velocity (double-C stencil) using the
            barycentric weight of the cell-centre vertex.

        ``"face_idw"``
            Inverse-distance-weighted average of all face-midpoint 2D
            velocities within the owning cell.

        ``"face_gradient"``
            Fit a local linear velocity gradient inside each cell from face
            midpoint velocities.  Cells with fewer than 3 faces fall back to
            the WLS cell-centre velocity.

        ``"facepoint_blend"``
            Assign velocity to every facepoint by averaging adjacent wet
            face velocities, then perform full 3-vertex barycentric
        ...
        ``"scatter_face"`` *(default)*
            Same as ``"scatter_cell_face"`` but using wet face midpoints only,
            eliminating discontinuities that can originate at cell centres.

        ``"scatter_corners"``
            Global scattered interpolation from wet mesh corners (facepoints).
            Full 2D velocity at each corner is computed as the unweighted mean
            of the double-C stencil face velocities at all adjacent wet faces.
            Provides denser, boundary-conforming scatter points without
            requiring *face_centers*.
    ...
    ImportError
        If ``rasterio`` or ``matplotlib`` are not installed.
    ValueError
        If *method* is not a recognised string, or if required arrays are
        missing for the chosen interpolation method.
    """
    _INTERP_METHODS = {
        "triangle_blend", "face_idw", "face_gradient",
        "facepoint_blend", "scatter_cell_face", "scatter_face",
    ...
    if method in _INTERP_METHODS and (
        face_normals is None or face_normal_velocity is None
    ):
        raise ValueError(
            f"method={method!r} requires face_normals and face_normal_velocity."
        )
    if method in (_INTERP_METHODS - {"scatter_corners"}) and face_centers is None:
        raise ValueError(
            f"method={method!r} requires face_centers."
        )
    ...
    if method == "flat_cell_center":
        ...
        if method == "scatter_corners":
            try:
                from scipy.interpolate import griddata
            except ImportError as exc:
                raise ImportError(
                    "scatter_corners requires scipy. "
                    "Install it with: pip install raspy[geo]"
                ) from exc
            ...
            pts     = facepoint_coords_arr[wet_fp]
            vel     = fp_vel[wet_fp]
        ...
        elif method == "scatter_corners_face":
            try:
                from scipy.interpolate import griddata
            except ImportError as exc:
                raise ImportError(
                    "scatter_corners_face requires scipy. "
                    "Install it with: pip install raspy[geo]"
                ) from exc
        ...
        elif method in {"scatter_cell_face", "scatter_face"}:
            try:
                from scipy.interpolate import griddata
            except ImportError as exc:
                raise ImportError(
                    "scatter_cell_face/scatter_face requires scipy. "
                    "Install it with: pip install raspy[geo]"
                ) from exc

            from raspy.hdf._velocity import compute_all_face_velocities
            ...
            #  Cell-local methods: fan-triangulate then trifinder.
            from raspy.hdf._velocity import compute_all_face_velocities
            ...
            try:
                ...
                raise ImportError(
                    "mesh_to_velocity_raster requires matplotlib for "
                    f"method={method!r}. Install it with: pip install raspy[geo]"
                ) from exc
            ...
            if not _trifinder_ok:
                ...
                if method == "triangle_blend":
                    ...
                    v_cell = np.where(
                        dry_mask[cell_idx_hit, np.newaxis], np.nan, v_cell
                    )
                    ...
                    if method == "face_idw":
                        v_pixel = _interp_face_idw(
                            ...
                            face_centroids_arr, face_vel_2d, dry_mask,
                        )
                    else:  # face_gradient
                        v_pixel = _interp_face_gradient(
                            px, py, cell_idx_hit, n_cells,
                            cell_centers_arr, cell_face_info_arr, cell_face_values_arr,
                            face_centroids_arr, face_vel_2d, dry_mask, cell_velocity,
                        )
...
def _shallow_cell_mask(
    cell_face_info: np.ndarray,
    cell_face_values: np.ndarray,
    facecenter_values: np.ndarray,
    face_min_elevation: np.ndarray,
) -> np.ndarray:
    """Return a bool mask: True where all face centroids of a cell are dry.
    ...
    cell_face_values : ndarray, shape ``(total_entries, 2)``
        ``[face_idx, orientation]`` per cell-face association.
    facecenter_values : ndarray, shape ``(n_faces,)``
        WSE at each face centroid.  ``NaN`` where the face is dry.
    face_min_elevation : ndarray, shape ``(n_faces,)``
        Minimum bed elevation (invert) at each face centroid.
    ...
    """
    depth = facecenter_values - face_min_elevation  # NaN propagates from fc_vals
    n_cells = len(cell_face_info)
    cell_is_shallow = np.ones(n_cells, dtype=bool)
    for i in range(n_cells):
        start, count = cell_face_info[i]
        if count == 0:
            continue
        face_idxs = cell_face_values[start : start + count, 0]
        d = depth[face_idxs]
        # shallow only if every face depth is <= 0 or NaN
        with np.errstate(invalid="ignore"):
            cell_is_shallow[i] = np.all((d <= 0) | np.isnan(d))
    return cell_is_shallow
